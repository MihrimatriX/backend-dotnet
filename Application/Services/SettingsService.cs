...
namespace EcommerceBackend.Application.Services
{
    public class SettingsService : ISettingsService
    {
        ...
        public Task<BaseResponseDto<string>> ImportSettingsAsync(int userId, string settingsJson)
        {
            try
            {
                _ = JsonSerializer.Deserialize<dynamic>(settingsJson);
                return Task.FromResult(BaseResponseDto<string>.SuccessResult("Settings imported successfully", "Settings imported successfully"));
            }
            catch (Exception ex)
            {
                return Task.FromResult(BaseResponseDto<string>.ErrorResult($"Error importing settings: {ex.Message}"));
            }
        }
    }
}

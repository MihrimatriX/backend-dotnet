...
namespace EcommerceBackend.Infrastructure.Data
{
    public class DataSeeder
    {
        ...
        public async Task SeedAsync()
        {
            ...
            await EnsureDenseProductsAsync();
            await AssignSubCategoriesToProductsAsync();
            ...
            await EnsureCampaignVolumeAsync();
            ...
            await EnsureReviewCoverageAsync();
            await EnsureStandardAccountsAsync();
        }
        ...
        private async Task SeedProductsAsync()
        {
            ...
            // Her kategoriden yoğun ürün (vitrin / sayfalama demo için)
        ...
        /// <summary>
        /// Sabit demo hesapları eksikse ekler (mevcut veritabanına idempotent).
        /// Şifreler appsettings ile değil seed ile tanımlıdır; üretimde değiştirin.
        /// </summary>
        private async Task EnsureStandardAccountsAsync()
        {
            var standard = new (string Email, string Password, string First, string Last)[]
            {
                ("manager@shop.demo", "Manager123!", "Panel", "Yönetici"),
                ("support@shop.demo", "Support123!", "Destek", "Uzman"),
                ("demo.buyer@shop.local", "Buyer123!", "Demo", "Alıcı"),
                ("staff@shop.demo", "Staff123!", "Mağaza", "Personel"),
            };

            foreach (var (email, password, first, last) in standard)
            {
                var lower = email.ToLowerInvariant();
                if (await _context.Users.AnyAsync(u => u.Email.ToLower() == lower))
                    continue;
        ...
        private async Task SeedUsersAsync()
        {
            var users = new List<User>
            {
                new()
                {
                    ...
                    Password = BCrypt.Net.BCrypt.HashPassword("admin123", BCrypt.Net.BCrypt.GenerateSalt(10)),
                    FirstName = "Admin",
                    LastName = "User",
                ...
                new()
                {
                    ...
                    Password = BCrypt.Net.BCrypt.HashPassword("user123", BCrypt.Net.BCrypt.GenerateSalt(10)),
                    FirstName = "Ahmet",
                    LastName = "Yılmaz",
        ...
        {
            var products = await _context.Products.OrderBy(p => p.Id).Take(100).ToListAsync();
        ...
        private const int MinimumProductCount = 2800;

        /// <summary>
        /// İlk kurulumda veya eksik DB’de katalog yoğunluğunu pazar yeri vitrinine uygun seviyeye çıkarır.
        /// </summary>
        private async Task EnsureDenseProductsAsync()
        {
            var count = await _context.Products.CountAsync();
            if (count >= MinimumProductCount)
            {
                return;
            }

            var categories = await _context.Categories.AsNoTracking().ToListAsync();
            var subs = await _context.SubCategories.AsNoTracking().ToListAsync();
            var byCat = subs
                .GroupBy(s => s.CategoryId)
                .ToDictionary(g => g.Key, g => g.ToList());

            var need = MinimumProductCount - count;
            var batch = new List<Product>(capacity: 512);
            var seq = count;

            for (var n = 0; n < need; n++)
            {
                var cat = categories[_random.Next(categories.Count)];
                byCat.TryGetValue(cat.Id, out var slist);
                var sub = slist is { Count: > 0 } ? slist[_random.Next(slist.Count)] : null;
                seq++;

                batch.Add(new Product
                {
                    ProductName = $"{cat.CategoryName} Çok Satan #{seq}",
                    UnitPrice = GenerateRandomPrice(),
                    UnitInStock = _random.Next(2, 500),
                    QuantityPerUnit = "1 adet",
                    CategoryId = cat.Id,
                    SubCategoryId = sub?.Id,
                    Description =
                        $"{cat.CategoryName} kategorisinde kampanyalı ürün. Orijinal, hızlı kargo ve kolay iade.",
                    ImageUrl = $"https://picsum.photos/seed/pk{seq % 8000}/400/400",
                    Discount = _random.Next(5, 60),
                    IsActive = true,
                    CreatedAt = DateTime.UtcNow.AddDays(-_random.Next(0, 120)),
        ...
        /// <summary>
        /// Ürünlere (mümkünse) aynı kategorideki alt kategori atar.
        /// </summary>
        private async Task AssignSubCategoriesToProductsAsync()
        {
            var subs = await _context.SubCategories.AsNoTracking().ToListAsync();
            if (subs.Count == 0)
            {
            ...
            var byCat = subs
                .GroupBy(s => s.CategoryId)
                .ToDictionary(g => g.Key, g => g.ToList());
            var catIds = byCat.Keys.ToHashSet();

            while (true)
            {
                var batch = await _context.Products
                    .Where(p => p.SubCategoryId == null && catIds.Contains(p.CategoryId))
                    .OrderBy(p => p.Id)
                    .Take(600)
                    .ToListAsync();
                ...
                {
                    if (!byCat.TryGetValue(p.CategoryId, out var list) || list.Count == 0)
                    {
                        continue;
                    }

                    p.SubCategoryId = list[_random.Next(list.Count)].Id;
                }
        ...
        private async Task EnsureCampaignVolumeAsync()
        {
            const int minCampaigns = 14;
            var n = await _context.Campaigns.CountAsync();
            if (n >= minCampaigns)
            {
                return;
            }

            var templates = new (string Title, string Sub, int Discount, string Color, string Href)[]
            {
                ("Süper Fırsat Gecesi", "Seçili ürünlerde ekstra indirim", 45, "#7c3aed", "/products"),
                ("Marka Günleri", "Sevilen markalar tek adreste", 30, "#db2777", "/products"),
                ("Outlet Köşesi", "Son parça fırsatları", 55, "#0d9488", "/products"),
                ("Ev & Yaşam Festivali", "Dekorasyon ve mutfak", 35, "#ea580c", "/products"),
                ("Teknoloji Şöleni", "Aksesuar ve gadget", 25, "#2563eb", "/products"),
                ("Kışa Özel", "Mont, bot ve termal", 40, "#0891b2", "/products"),
                ("Anne & Bebek Günleri", "Bebek bakımında indirim", 28, "#c026d3", "/products"),
                ("Spor Festivali", "Fitness ve outdoor", 32, "#16a34a", "/products"),
                ("Kozmetik Karnavalı", "Parfüm ve bakım", 22, "#f59e0b", "/products"),
                ("Kitap & Hobi", "Okuma ve koleksiyon", 18, "#64748b", "/products"),
            };

            var extra = new List<Campaign>();
            for (var i = n; i < minCampaigns; i++)
            {
                var t = templates[i % templates.Length];
                extra.Add(new Campaign
                {
                    Title = $"{t.Title} #{i + 1}",
                    Subtitle = t.Sub,
                    Description = $"{t.Sub}. Stoklar tükenmeden yakalayın.",
                    Discount = t.Discount,
                    ImageUrl = $"https://picsum.photos/800/400?random={2000 + i}",
                    BackgroundColor = t.Color,
                    TimeLeft = $"{(i % 5) + 2} gün",
                    ButtonText = "İncele",
                    ButtonHref = t.Href,
                    StartDate = DateTime.UtcNow.AddDays(-(i % 3)),
                    EndDate = DateTime.UtcNow.AddDays(10 + i),
                    IsActive = true
                });
            }

            await _context.Campaigns.AddRangeAsync(extra);
            await _context.SaveChangesAsync();
        }

        private async Task EnsureReviewCoverageAsync()
        {
            const int targetTotal = 3200;
            var current = await _context.Reviews.CountAsync();
            ...
            var pairs = await _context.Reviews
                .Select(r => new { r.UserId, r.ProductId })
                .ToListAsync();
            var used = pairs.Select(x => (x.UserId, x.ProductId)).ToHashSet();

            var reviewTitles = new[]
            {
                "Harika ürün!", "Çok memnun kaldım", "Tavsiye ederim", "Hızlı kargo",
                "Fiyat performans iyi", "Kaliteli", "Beklentinin üstünde", "Güvenle alınır"
            };

            var reviewComments = new[]
            {
                "Ürün açıklamaya uygun, kargo hızlıydı.",
                "Paketleme özenli, teşekkürler.",
                "Fiyatına göre çok iyi, tekrar alırım.",
                "Genel olarak memnunum, tavsiye ederim."
            };

            var toAdd = new List<Review>(512);
            var rnd = _random;
            var guard = 0;
            while (current + toAdd.Count < targetTotal && guard < 80_000)
            {
                guard++;
                var uid = userIds[rnd.Next(userIds.Count)];
                ...
                toAdd.Add(new Review
                {
                    ...
                    IsVerified = rnd.Next(3) != 0,
                    IsHelpful = rnd.Next(2) == 1,
                    IsActive = true,
                    CreatedAt = DateTime.UtcNow.AddDays(-rnd.Next(1, 400)),
                    UpdatedAt = DateTime.UtcNow.AddDays(-rnd.Next(1, 200))
                });
                ...
                await _context.Reviews.AddRangeAsync(toAdd);
                await _context.SaveChangesAsync();
                current += toAdd.Count;
                toAdd.Clear();
            }

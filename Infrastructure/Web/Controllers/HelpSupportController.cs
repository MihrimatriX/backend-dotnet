...
namespace EcommerceBackend.Infrastructure.Web.Controllers
{
    ...
    public class HelpSupportController : ControllerBase
    {
        ...
        [Authorize(Roles = "Admin")]
        public async Task<ActionResult<BaseResponseDto<List<SupportTicketDto>>>> GetAllSupportTicketsForAdmin(
            [FromQuery] int pageNumber = 1,
            [FromQuery] int pageSize = 20)
        {
            try
            {
                var result = await _helpSupportService.GetAllSupportTicketsAdminAsync(pageNumber, pageSize);
                if (result.Success)
                {
                    return Ok(result);
                }
